...
public:
  ...
  Token next() {
    ...
    if (std::isalpha(c)) {
      ...
      if (id == "sin")
        return {FUNC_SIN, id, 0};
      if (id == "cos")
        return {FUNC_COS, id, 0};
      if (id == "tan")
        return {FUNC_TAN, id, 0};
      if (id == "log")
        return {FUNC_LOG, id, 0};
      if (id == "exp")
        return {FUNC_EXP, id, 0};
      if (id == "sqrt")
        return {FUNC_SQRT, id, 0};
...
public:
  Parser(const std::string &text) : lexer(text) { current = lexer.next(); }
...
const char *get_version() { return "Equation Solver v1.0"; }
